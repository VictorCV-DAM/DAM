...
//    1R LISTADO DE DIRECTORIOS------------------------------------

    val s = "Llista de fitxers i directoris del directori " + f
    println(s)
...
//                println("El valor no es numeric i no es valid")
            ...
            if (v == -1) {
                    if ( f.canonicalPath != "/"){
                        val dirPadre= f.parentFile
                        if (dirPadre.exists()) {
                            llistaDirectori(dirPadre)
                    }
                }
            }else

                if (v<=25) {
                    val peticion = File(f.listFiles().sorted()[v].canonicalPath)
...
fun seleccionaDir(arch: File) {
    ...
    val arr2 = arch.listFiles().sorted()

    if (arch.isDirectory){
    println(arch)

    llistaDirectori(arch)
    }
